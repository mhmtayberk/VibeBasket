...
- **Typecheck Reality**: `packages/registry` now has a dedicated cross-package typecheck config and passes its own strict check, but a monorepo-wide `pnpm typecheck` still fails in `packages/adapters` test fixtures because newer MCP metadata now requires stricter fields like `headers`.
