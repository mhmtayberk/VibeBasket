...
- **Validation**: Zod.
- **Database**: Drizzle ORM with LibSQL/SQLite for catalog, bundles, auth, stacks, sync runs, and backup storage config. FTS5 full-text search with trigger-based content sync and rebuild support.
...
- Catalog search uses FTS5 full-text indexing with automatic trigger-based content sync.
