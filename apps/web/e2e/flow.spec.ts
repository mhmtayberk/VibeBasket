...
test.describe("VibeBasket — Catalog API", () => {
  test("GET /api/catalog returns paginated JSON", async ({ page }) => {
    const res = await page.goto("/api/catalog?type=mcp&limit=5");
  ...
  test("catalog has ALL 4 item types", async ({ page }) => {
    for (const t of ["mcp", "skill", "rule", "workflow"]) {
      const res = await page.goto(`/api/catalog?type=${t}&limit=3`);
      const body = await res?.json();
      expect(res?.status()).toBe(200);
  ...
  test("catalog trust filter: verified", async ({ page }) => {
    const res = await page.goto("/api/catalog?trust=verified&limit=5");
    const body = await res?.json();
    expect(res?.status()).toBe(200);
  ...
  test("catalog trust filter: official returns non-verified from official sources", async ({
    page,
  }) => {
    const res = await page.goto("/api/catalog?trust=official&limit=10");
    const body = await res?.json();
    expect(res?.status()).toBe(200);
    for (const item of body.items) {
  ...
  test("catalog freshness filters work", async ({ page }) => {
    for (const freshness of ["fresh", "recent", "aging"]) {
      const res = await page.goto(`/api/catalog?freshness=${freshness}&limit=3`);
  ...
  test("catalog sort options return valid results", async ({ page }) => {
    for (const sort of ["freshest", "name", "verified"]) {
      const res = await page.goto(`/api/catalog?sort=${sort}&limit=5`);
  ...
  test("catalog pagination: page 2 has different items than page 1", async ({ page }) => {
    const [p1, p2] = await Promise.all([
      page.goto("/api/catalog?page=1&limit=10").then((r) => r?.json()),
      page.goto("/api/catalog?page=2&limit=10").then((r) => r?.json()),
    ]);
    const ids1 = new Set((p1 as CatalogApiResponse).items.map((i) => i.id));
    const ids2 = new Set((p2 as CatalogApiResponse).items.map((i) => i.id));
    const overlap = [...ids1].filter((id) => ids2.has(id));
    expect(overlap.length).toBe(0);
  });

  test("catalog pagination: beyond last page returns empty", async ({ page }) => {
    const res = await page.goto("/api/catalog?page=99999&limit=10");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
  ...
  test("catalog limit is capped at MAX_LIMIT=100", async ({ page }) => {
    const res = await page.goto("/api/catalog?limit=500");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
  ...
  test("catalog invalid page (0 or negative) defaults to page 1", async ({ page }) => {
    for (const badPage of ["0", "-1", "abc"]) {
      const res = await page.goto(`/api/catalog?page=${badPage}&limit=5`);
      expect(res?.status()).toBe(200);
      const body = await res?.json();
  ...
  test("catalog invalid type returns items unfiltered", async ({ page }) => {
    const res = await page.goto("/api/catalog?type=nonexistent");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
    expect(body.items).toBeDefined();
  });
...
test.describe("VibeBasket — Search (+ FTS5)", () => {
  test("search returns relevant results", async ({ page }) => {
    const res = await page.goto("/api/catalog?q=github&limit=10");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
  ...
  test("search is case-insensitive", async ({ page }) => {
    const [lower, upper] = await Promise.all([
      page.goto("/api/catalog?q=github&limit=5").then((r) => r?.json()),
      page.goto("/api/catalog?q=GITHUB&limit=5").then((r) => r?.json()),
    ]);
    const lowerIds = new Set((lower as CatalogApiResponse).items.map((i) => i.id));
    const upperIds = new Set((upper as CatalogApiResponse).items.map((i) => i.id));
    expect(lowerIds.size).toBe(upperIds.size);
  });

  test("search with zero results returns empty array", async ({ page }) => {
    const res = await page.goto("/api/catalog?q=zzzxxxxyynonexistent123456&limit=5");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
    expect(body.items.length).toBe(0);
  });
  ...
  test("search query longer than MAX_SEARCH_LENGTH is truncated", async ({ page }) => {
    ...
    const res = await page.goto(`/api/catalog?q=${longQ}&limit=5`);
    expect(res?.status()).toBe(200);
  });

  test("search with special characters is sanitized", async ({ page }) => {
    const res = await page.goto(
      `/api/catalog?q=${encodeURIComponent("<script>alert(1)</script>")}&limit=5`,
    );
    expect(res?.status()).toBe(200);
  });

  test("search with whitespace-only query returns items", async ({ page }) => {
    const res = await page.goto("/api/catalog?q=+++&limit=5");
    expect(res?.status()).toBe(200);
  });

  test("search combined with type filter", async ({ page }) => {
    const res = await page.goto("/api/catalog?q=server&type=mcp&limit=5");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
    for (const item of body.items) {
  ...
  test("search combined with trust filter does not break", async ({ page }) => {
    const res = await page.goto("/api/catalog?q=github&trust=verified&limit=5");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
    for (const item of body.items) {
      expect(item.verified).toBe(true);
  ...
  test("search with FTS5 prefix match returns results", async ({ page }) => {
    ...
    const res = await page.goto("/api/catalog?q=postgre&limit=5");
    expect(res?.status()).toBe(200);
    const body = await res?.json();
  ...
  test("search multi-word query returns intersection", async ({ page }) => {
    const res = await page.goto(`/api/catalog?q=${encodeURIComponent("github mcp")}&limit=10`);
    expect(res?.status()).toBe(200);
  });
});
...
test.describe("VibeBasket — Security Headers", () => {
  ...
  test("non-existent API route returns 404", async ({ page }) => {
    const res = await page.goto("/api/nonexistent-endpoint-xyz");
...
test.describe("VibeBasket — Docs Pages", () => {
  ...
  test("invalid tab defaults to hub content", async ({ page }) => {
    await page.goto("/docs?tab=invalidtab123");
...
test.describe("VibeBasket — Error States", () => {
  test("missing bundle ID returns 404", async ({ page }) => {
    const res = await page.goto("/api/bundle/nonexistent-id-12345");
    expect(res?.status()).toBe(404);
  });

  test("bundle POST without body returns 400/405", async ({ page }) => {
    const res = await page.goto("/api/bundle", { waitUntil: "commit" });
