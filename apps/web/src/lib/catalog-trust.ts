...
function sourceLabel(sourceName?: string | null) {
	switch (sourceName) {
		case "verified-catalog": return "Curated by VibeBasket";
		case "official-mcp-registry": return "Official MCP Registry";
		case "skills-sh-official": return "skills.sh Official";
		case "skills-sh-community": return "skills.sh Community";
...
export function withCatalogTrust<T extends { id: string; name: string; description: string; type: string; icon?: string; mcpData?: unknown }>(
	item: T,
	source: CatalogLikeItem,
): T & { trust: CatalogTrust } {
	return {
		...item,
