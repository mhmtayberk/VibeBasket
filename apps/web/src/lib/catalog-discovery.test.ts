...
describe("matchesCatalogTrustFilter", () => {
	it("treats verified items as their own lane", () => {
		expect(
			matchesCatalogTrustFilter(
				{ verified: true, sourceName: "verified-catalog" },
				"verified",
			),
		).toBe(true);
		expect(
	...
	it("recognizes official upstream sources", () => {
		for (const sourceName of OFFICIAL_SOURCE_NAMES) {
			expect(
				matchesCatalogTrustFilter({ verified: false, sourceName }, "official"),
			).toBe(true);
		}
		expect(
			matchesCatalogTrustFilter(
				{ verified: false, sourceName: "community-feed" },
				"official",
			),
		).toBe(false);
	});
...
describe("classifyCatalogFreshness", () => {
	...
	it("classifies fresh, recent, and aging windows", () => {
		expect(classifyCatalogFreshness(now - 4 * 60 * 60 * 1000, now)).toBe(
			"fresh",
		);
...
describe("matchesCatalogFreshnessFilter", () => {
	...
	it("matches the recent window without including fresh rows", () => {
		expect(
			matchesCatalogFreshnessFilter(
				{ lastSyncedAt: new Date(now - 3 * DAY_MS) },
				"recent",
				now,
			),
		).toBe(true);
		expect(
			matchesCatalogFreshnessFilter(
				{ lastSyncedAt: new Date(now - 6 * 60 * 60 * 1000) },
				"recent",
				now,
			),
		).toBe(false);
