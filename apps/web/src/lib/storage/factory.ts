...
const STORAGE_BACKEND_OPTIONS: Array<{
	...
	},
	{
		...
		label: "Cloudflare R2",
		description:
			"Cloudflare R2 object storage — S3-compatible (BACKUP_R2_*)",
		envPrefix: "BACKUP_R2",
	},
	{
		id: "spaces",
		...
		description:
			"DigitalOcean Spaces — S3-compatible (BACKUP_SPACES_*)",
...
function isS3ConfigComplete(
	config: ReturnType<typeof buildS3Config>,
): boolean {
	return Boolean(
...
async function createS3BackendFromConfig(
	id: "s3" | "r2" | "spaces",
	label: string,
	s3Config: { endpoint: string; region?: string; bucket: string; accessKey: string; secretKey: string },
...
async function createAzureBackendFromConfig(
...
async function createGcsBackendFromConfig(
	config: { bucket: string; projectId: string },
): Promise<StorageBackend | null> {
		...
		switch (id) {
			...
			case "spaces": {
				...
				if (!s3cfg?.endpoint || !s3cfg?.bucket || !s3cfg?.accessKey || !s3cfg?.secretKey)
					return null;
				return createS3BackendFromConfig(id, STORAGE_BACKEND_OPTIONS.find(o => o.id === id)?.label ?? id, s3cfg);
			}
			case "azure": {
...
export function getAllBackendsStatus(): BackendStatus[] {
	const activeId = resolveBackendId();
	...
	return STORAGE_BACKEND_OPTIONS.map((opt) => {
		...
		switch (opt.id) {
			...
			case "azure":
				...
				if (!readEnv("BACKUP_AZURE_CONTAINER"))
					missingVars.push("BACKUP_AZURE_CONTAINER");
				break;
			case "gcs":
				if (!readEnv("BACKUP_GCS_BUCKET"))
					missingVars.push("BACKUP_GCS_BUCKET");
				if (!readEnv("BACKUP_GCS_PROJECT_ID"))
					missingVars.push("BACKUP_GCS_PROJECT_ID");
				break;
		}
