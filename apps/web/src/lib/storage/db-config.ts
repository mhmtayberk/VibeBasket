...
export async function deleteStorageConfig(): Promise<void> {
  await db.delete(backupStorageConfig).where(eq(backupStorageConfig.id, "default"));
}
...
export async function resolveBackendId(): Promise<StorageBackendId> {
  ...
  if (STORAGE_BACKEND_OPTIONS.some((opt) => opt.id === envBackend)) {
...
export async function isScheduleDue(): Promise<boolean> {
  ...
  const lastAt = schedule.lastScheduledAt ? new Date(schedule.lastScheduledAt).getTime() : 0;
