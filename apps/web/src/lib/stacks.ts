...
export const createStackSchema = z.object({
  ...
  description: z.string().trim().max(280).optional().transform((value) => value || undefined),
...
export function normalizeStackTargetIds(targetIds: string[]) {
  ...
  const unsupportedTargetIds = uniqueTargetIds.filter((targetId) => !SUPPORTED_TARGET_IDS.includes(targetId));

  if (unsupportedTargetIds.length > 0) {
    throw new Error(`Unsupported target IDs: ${unsupportedTargetIds.join(", ")}`);
  }
