...
export const TRUST_FILTER_OPTIONS: Array<{ value: CatalogTrustFilter; label: string }> = [
...
export const FRESHNESS_FILTER_OPTIONS: Array<{ value: CatalogFreshnessFilter; label: string }> = [
...
const TRUST_FILTER_VALUES: CatalogTrustFilter[] = ["all", "verified", "official", "community"];
const FRESHNESS_FILTER_VALUES: CatalogFreshnessFilter[] = ["all", "fresh", "recent", "aging"];
...
function isTrustFilter(value: string | null | undefined): value is CatalogTrustFilter {
  return TRUST_FILTER_VALUES.includes((value ?? "") as CatalogTrustFilter);
}

function isFreshnessFilter(value: string | null | undefined): value is CatalogFreshnessFilter {
  return FRESHNESS_FILTER_VALUES.includes((value ?? "") as CatalogFreshnessFilter);
}
...
export function normalizeCatalogDiscoveryInput(
  input: Partial<Record<keyof CatalogDiscoveryState, string | null | undefined>>
): CatalogDiscoveryState {
  const defaults = getCatalogDiscoveryDefaults();

  return {
    trust: isTrustFilter(input.trust) ? input.trust : defaults.trust,
    freshness: isFreshnessFilter(input.freshness) ? input.freshness : defaults.freshness,
    sort: isSortOption(input.sort) ? input.sort : defaults.sort,
  };
}

export function isOfficialCatalogSource(sourceName?: string | null) {
  return OFFICIAL_SOURCE_NAMES.includes((sourceName ?? "") as (typeof OFFICIAL_SOURCE_NAMES)[number]);
}
