...
export async function GET(
	...
	try {
		...
		return NextResponse.json(
			{ error: "Failed to fetch saved stack." },
			{ status: 500 },
		);
	}
}
...
export async function PATCH(
	...
	try {
			...
			const conflictingStack = await db
				...
				.where(
					and(
						...
						eq(savedStacks.name, payload.name),
					),
				)
				.limit(1);
		...
		return NextResponse.json(
			{ error: "Failed to update saved stack." },
			{ status: 500 },
		);
	}
}
...
export async function DELETE(
	...
	try {
		...
		return NextResponse.json(
			{ error: "Failed to delete saved stack." },
			{ status: 500 },
		);
	}
}
