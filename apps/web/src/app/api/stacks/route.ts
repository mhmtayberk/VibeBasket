...
export async function GET(request: NextRequest, _context: RouteContext<"/api/stacks">) {
  ...
  try {
    ...
    const rawLimit =
      Number.parseInt(searchParams.get("limit") || `${DEFAULT_STACKS_LIMIT}`, 10) ||
      DEFAULT_STACKS_LIMIT;
...
export async function POST(request: NextRequest, _context: RouteContext<"/api/stacks">) {
  ...
  try {
    ...
    const now = new Date();
    const catalogSelectionById = new Map(catalogSelection.map((item) => [item.id, item] as const));
