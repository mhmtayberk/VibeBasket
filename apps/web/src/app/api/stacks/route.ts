...
export async function GET(_request: NextRequest, _context: RouteContext<"/api/stacks">) {
  try {
    ...
    return NextResponse.json({
      stacks: stacks.map((stack) => ({
        ...
        targetIds: (targetsByStackId.get(stack.id) ?? []).map((target) => target.targetId),
      })),
    });
  } catch (error) {
    ...
    return NextResponse.json({ error: "Failed to list saved stacks." }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest, _context: RouteContext<"/api/stacks">) {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.insert(savedStackItems).values(
        itemIds.map((itemId, index) => {
          const item = catalogSelection.find((entry) => entry.id === itemId)!;
          return {
            id: nanoid(14),
            stackId,
    ...
    if (error instanceof Error && error.message.startsWith("Unsupported target IDs:")) {
      return NextResponse.json({ error: error.message }, { status: 400 });
    }

    console.error("Failed to create saved stack:", error);
    return NextResponse.json({ error: "Failed to create saved stack." }, { status: 500 });
  }
}
