...
const cleanupStaleDataMock = vi.fn();
...
vi.mock("@/lib/data-cleanup", () => ({
  cleanupStaleData: cleanupStaleDataMock,
}));
...
describe("GET /api/bundle/[id]", () => {
  ...
  it("returns the stored manifest when the bundle exists", async () => {
    ...
    const response = await GET(new Request("http://localhost:3000/api/bundle/bundle-123456"), {
  ...
  it("returns 404 when the bundle does not exist", async () => {
    ...
    const { GET } = await import("./route");
    const response = await GET(new Request("http://localhost:3000/api/bundle/missing"), {
