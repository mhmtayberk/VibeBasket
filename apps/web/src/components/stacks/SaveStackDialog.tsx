...
export function SaveStackDialog({ disabled, items, targetIds, onSaved }: SaveStackDialogProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md border border-border/80 bg-card/95 p-0 text-foreground ring-0">
        <DialogHeader className="border-b border-border/70 px-5 py-5">
          <DialogTitle className="text-xl font-semibold">Save current basket</DialogTitle>
          <DialogDescription className="max-w-sm text-sm leading-6 text-muted-foreground">
            Capture your current basket and target IDE selection so you can reopen it anytime.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <div className="grid grid-cols-2 gap-3 border border-border/60 bg-background/30 p-3">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-foreground">{items.length}</p>
            </div>
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
                Targets
              </p>
              <p className="mt-1 text-sm font-medium text-foreground">{targetIds.length}</p>
            </div>
          </div>
