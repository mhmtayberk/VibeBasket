...
import { Loader2, Plus, Save, TerminalSquare, X } from "lucide-react";
...
function typeBadge(type: string) {
  ...
  return (
    <span
      className={`shrink-0 font-mono text-[10px] uppercase tracking-wider px-2.5 py-1 border ${colors}`}
    >
      {type}
    </span>
  );
        ...
        <div className="flex items-center justify-between border-b border-border/70 px-8 py-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {stack.name} — {items.length} item{items.length !== 1 ? "s" : ""} · {targetIds.length}{" "}
              target{targetIds.length !== 1 ? "s" : ""}
            </p>
          </div>
          <button
            type="button"
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setItems(
                    basket.map((b) => ({
                      catalogItemId: b.id,
                      snapshotDisplayName: b.name,
                      catalogItemType: b.type,
                    })),
                  );
            ...
            <div className="space-y-5 min-w-0">
              ...
              {items.map((item) => (
                <div
                  ...
                  className="flex items-center justify-between gap-4 border border-border/50 bg-background/30 pl-4 pr-2 py-3.5 group"
                >
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setItems((p) => p.filter((i) => i.catalogItemId !== item.catalogItemId))
                    }
              ...
              {items.length === 0 && (
                <div className="border border-dashed border-border/50 p-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/60 mt-2">
                    Add from your active basket below or browse the catalog.
                  </p>
                </div>
              )}
                ...
                {supportedTargets.map((target) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setTargetIds((p) =>
                          p.includes(target.id)
                            ? p.filter((id) => id !== target.id)
                            : [...p, target.id],
                        )
                      }
