				...
				<div className="sticky top-0 z-10 flex items-start justify-between border-b border-border/70 bg-card px-5 sm:px-6 py-4">
					<div className="min-w-0 pr-4">
						<div className="flex items-center gap-2 mb-1">
							<span className={`font-mono text-[10px] uppercase tracking-[0.16em] px-2 py-0.5 border shrink-0 ${
								item.type === "mcp" ? "border-orange-500/30 bg-orange-500/10 text-orange-400" : "border-accent/30 bg-accent/10 text-accent"
							}`}>{item.type}</span>
							{item.trust && (
								<span className={`font-mono text-[9px] uppercase tracking-[0.14em] px-1.5 py-0.5 border ${
									item.trust.tier === "verified" ? "border-accent/30 bg-accent/10 text-accent"
									: item.trust.tier === "official" ? "border-blue-400/30 bg-blue-400/10 text-blue-400"
									: "border-border/50 bg-background/30 text-muted-foreground"
								}`}>{item.trust.label}</span>
							)}
						</div>
				...
				<div className="px-5 sm:px-6 py-5 space-y-5">
					...
					{item.trust && (
						<div className="flex items-center gap-2 text-xs text-muted-foreground">
							...
							<span>{item.trust.sourceLabel} · {item.trust.detail}</span>
						</div>
					)}
					...
					{mcpData && item.type === "mcp" && (
						<div className="space-y-2">
							<h3 className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Install Command</h3>
							<div className="border border-border/50 bg-background/30 p-4 font-mono text-[11px] text-foreground">
								<div>
									...
									{mcpData.command ? <span className="text-muted-foreground"> {mcpData.command}</span> : null}
									{mcpData.args?.length ? <span className="text-muted-foreground"> {mcpData.args.join(" ")}</span> : null}
								</div>
								{mcpData.url && (
									<div className="mt-1">
										<span className="text-muted-foreground">url:</span>{" "}
										<a href={mcpData.url as string} target="_blank" rel="noopener noreferrer"
							...
							{mcpData.requiredSecrets?.length ? (
								<p className="text-[10px] text-muted-foreground/70 leading-relaxed">
									Requires secrets: {mcpData.requiredSecrets.join(", ")}. Prompts locally by CLI. Never sent to servers.
								</p>
							) : null}
						</div>
					)}
					...
					{skillSource && item.type === "skill" && (
						<div className="space-y-2">
							<h3 className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Source</h3>
								...
								{isGithubSource(skillSource) && (
									<>
										<div>
											...
											<a href={`https://github.com/${skillSource.repo}`} target="_blank" rel="noopener noreferrer"
												className="text-accent hover:underline break-all">github.com/{skillSource.repo}</a>
										</div>
										{skillSource.path && <div><span className="text-muted-foreground">path</span> <span className="text-foreground">{skillSource.path}</span></div>}
										{skillSource.ref && skillSource.ref !== "main" && <div><span className="text-muted-foreground">ref</span> <span className="text-foreground">{skillSource.ref}</span></div>}
									</>
								)}
