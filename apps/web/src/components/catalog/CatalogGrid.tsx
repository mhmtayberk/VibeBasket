...
async function fetchCatalog(
	...
	try {
		...
		const payloadItems = Array.isArray(payload.items)
			? (payload.items as CatalogApiItem[])
			: [];
		...
		if (error instanceof DOMException && error.name === "AbortError") {
			throw new Error(
				`Catalog request timed out after ${REQUEST_TIMEOUT_MS / 1000}s`,
			);
		}
		throw error;
	} finally {
...
export function CatalogGrid({
	...
	const [trustFilter, setTrustFilter] = useState<CatalogTrustFilter>(
		defaultDiscovery.trust,
	);
	const [sortOption, setSortOption] = useState<CatalogSort>(
		defaultDiscovery.sort,
	);
	...
	const activeFilterSummaries = [
		trustFilter !== defaultDiscovery.trust
			? TRUST_FILTER_OPTIONS.find((option) => option.value === trustFilter)
					?.label
			: null,
			...
			<div className="mx-auto max-w-[1440px] px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
				<div className="max-w-3xl">
					...
					<p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">
						Browse trusted components, assemble your basket, and generate a
						single install command for the editors your team actually uses.
					</p>
				</div>
															...
															<div className="mt-2 flex flex-wrap gap-2">
																{group.options.map((option) => (
																	<button
																		...
																		type="button"
																		onClick={() =>
																			group.onChange(option.value as never)
																		}
																		className={cn(
																			"border px-3 py-2 font-mono text-[11px] uppercase tracking-[0.18em] transition-colors",
								...
								{loading ? (
									...
									<div className="flex min-h-[420px] flex-col items-center justify-center gap-3 border border-dashed border-border/70 bg-background/20 px-6 text-center">
										...
										<p className="max-w-md text-sm text-muted-foreground">
											Try a broader search term or switch to another catalog
											category.
										</p>
									</div>
								)}
							...
							<div className="border-t border-border/70 px-4 py-4 sm:px-5">
								<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
									<p className="text-sm text-muted-foreground">
										Large catalogs stay fast by loading only the current page
										and active category.
									</p>
