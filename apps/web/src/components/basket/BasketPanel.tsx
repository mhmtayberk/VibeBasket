...
export function BasketPanel({
	...
	const supportedTargets = TARGET_OPTIONS.filter(
				...
				<div className="space-y-3">
					{items.length === 0 ? (
						<div className="border border-dashed border-border/70 px-4 py-8 text-center text-sm text-muted-foreground">
							Select MCPs, skills, and rules from the builder to assemble your
							setup.
						</div>
					) : (
						<>
								...
								{(["mcp", "skill", "rule", "workflow"] as const).map((type) =>
									itemCounts[type] ? (
										...
										>
											...
											<span>
												{type === "workflow" ? "workflow packs" : `${type}s`}
											</span>
										</div>
									) : null,
								)}
							...
							<div className="space-y-3">
								...
								<div className="space-y-1 text-[11px]">
									...
									<p className="text-accent">
										&gt; Ready to apply across your selected IDEs.
									</p>
								</div>
							</div>
						) : (
