...
import {
	...
	GitHubCopilotAdapter, VoidAdapter, AiderAdapter,
	CortexCodeAdapter, GooseAdapter, IBMBobAdapter, CodeBuddyAdapter,
	...
	for (const [name, adapter] of ADAPTERS) {
		...
		if (hasConfig) {
			...
			try {
				...
				const mcpServers = (config as Record<string, unknown>)?.mcpServers || (config as Record<string, unknown>)?.mcp_servers as Record<string, unknown> | undefined;
				if (mcpServers && typeof mcpServers === "object") {
					mcps = Object.keys(mcpServers).length;
					mcpTotal += mcps;
				}
			} catch { /* read error, skip count */ }
		}
		...
		console.log(`  ${statusIcon} ${chalk.bold(name)}${mcpStr}${caps.length > 0 ? chalk.gray(`  [${caps.join(", ")}]`) : ""}`);
	}
