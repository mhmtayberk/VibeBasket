...
describe("CortexCodeAdapter", () => {
	...
	it("resolves project config path", () => {
		expect(adapter.configPath("project", tmpDir)).toBe(path.join(tmpDir, ".snowflake", "cortex", "mcp.json"));
	});
	...
	it("applies MCP servers to config", () => {
		...
		const mcps = [{ id: "test-mcp", displayName: "Test", runtime: "npx" as const, command: "test-cmd", args: [], env: {}, requiredSecrets: [], verified: false }];
